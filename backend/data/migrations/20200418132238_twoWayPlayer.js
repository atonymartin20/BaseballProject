
exports.up = function(knex, Promise) {
    return knex.schema.createTable('twoWayPlayer', twoWayPlayer => {
        twoWayPlayer.increments();
        twoWayPlayer.string('firstName', 50);
        twoWayPlayer.string('lastName', 50);
        twoWayPlayer.string('birthday', 50);
        twoWayPlayer.string('bats', 1);
        twoWayPlayer.string('throws', 1);
        twoWayPlayer.string('currentTeam', 50);
        twoWayPlayer.string('imageURL', 500);
        twoWayPlayer.string('primaryPosition', 50);
        twoWayPlayer.string('otherPositions', 50);
        twoWayPlayer.integer('GamesPlayed2017');
        twoWayPlayer.decimal('BA2017', 8, 3);
        twoWayPlayer.decimal('OBP2017', 8, 3);
        twoWayPlayer.decimal('SLG2017', 8, 3);
        twoWayPlayer.integer('PA2017');
        twoWayPlayer.integer('Doubles2017');
        twoWayPlayer.integer('HR2017');
        twoWayPlayer.integer('Runs2017');
        twoWayPlayer.integer('RBI2017');
        twoWayPlayer.string('LineupSlot2017', 50);
        twoWayPlayer.string('GBPercent2017', 50);
        twoWayPlayer.string('LDPercent2017', 50);
        twoWayPlayer.string('FBPercent2017', 50);
        twoWayPlayer.string('PullPercent2017', 50);
        twoWayPlayer.string('CenterPercent2017', 50);
        twoWayPlayer.string('OppoPercent2017', 50);
        twoWayPlayer.string('HRPerFB2017', 50);
        twoWayPlayer.string('HardHitPercent2017', 50);
        twoWayPlayer.decimal('ExitVelo2017', 8, 1);
        twoWayPlayer.decimal('FlyBallLineDriveExitVelo2017', 8, 1);
        twoWayPlayer.decimal('LaunchAngle2017', 8, 1);
        twoWayPlayer.string('BarrelPercent2017', 50);
        twoWayPlayer.decimal('BABIP2017', 8, 3);
        twoWayPlayer.decimal('WOBA2017', 8, 3);
        twoWayPlayer.decimal('xWOBA2017', 8, 3);
        twoWayPlayer.decimal('xBA2017', 8, 3);
        twoWayPlayer.integer('AvgHRDistance2017');
        twoWayPlayer.integer('WRCPlus2017');
        twoWayPlayer.integer('OPSPlus2017');
        twoWayPlayer.string('BBPercent2017', 50);
        twoWayPlayer.string('KPercent2017', 50);
        twoWayPlayer.string('ChaseRate2017', 50);
        twoWayPlayer.string('ContactPercent2017', 50);
        twoWayPlayer.decimal('SprintSpeed2017', 8, 1);
        twoWayPlayer.integer('StolenBases2017');
        twoWayPlayer.string('StolenBasePercent2017', 50);
        twoWayPlayer.decimal('FWAR2017', 8, 1);
        twoWayPlayer.integer('GamesPlayed2018');
        twoWayPlayer.decimal('BA2018', 8, 3);
        twoWayPlayer.decimal('OBP2018', 8, 3);
        twoWayPlayer.decimal('SLG2018', 8, 3);
        twoWayPlayer.integer('PA2018');
        twoWayPlayer.integer('Doubles2018');
        twoWayPlayer.integer('HR2018');
        twoWayPlayer.integer('Runs2018');
        twoWayPlayer.integer('RBI2018');
        twoWayPlayer.string('LineupSlot2018', 50);
        twoWayPlayer.string('GBPercent2018', 50);
        twoWayPlayer.string('LDPercent2018', 50);
        twoWayPlayer.string('FBPercent2018', 50);
        twoWayPlayer.string('PullPercent2018', 50);
        twoWayPlayer.string('CenterPercent2018', 50);
        twoWayPlayer.string('OppoPercent2018', 50);
        twoWayPlayer.string('HRPerFB2018', 50);
        twoWayPlayer.string('HardHitPercent2018', 50);
        twoWayPlayer.decimal('ExitVelo2018', 8, 1);
        twoWayPlayer.decimal('FlyBallLineDriveExitVelo2018', 8, 1);
        twoWayPlayer.decimal('LaunchAngle2018', 8, 1);
        twoWayPlayer.string('BarrelPercent2018', 50);
        twoWayPlayer.decimal('BABIP2018', 8, 3);
        twoWayPlayer.decimal('WOBA2018', 8, 3);
        twoWayPlayer.decimal('xWOBA2018', 8, 3);
        twoWayPlayer.decimal('xBA2018', 8, 3);
        twoWayPlayer.integer('AvgHRDistance2018');
        twoWayPlayer.integer('WRCPlus2018');
        twoWayPlayer.integer('OPSPlus2018');
        twoWayPlayer.string('BBPercent2018', 50);
        twoWayPlayer.string('KPercent2018', 50);
        twoWayPlayer.string('ChaseRate2018', 50);
        twoWayPlayer.string('ContactPercent2018', 50);
        twoWayPlayer.decimal('SprintSpeed2018', 8, 1);
        twoWayPlayer.integer('StolenBases2018');
        twoWayPlayer.string('StolenBasePercent2018', 50);
        twoWayPlayer.decimal('FWAR2018', 8, 1);
        twoWayPlayer.integer('GamesPlayed2019');
        twoWayPlayer.decimal('BA2019', 8, 3);
        twoWayPlayer.decimal('OBP2019', 8, 3);
        twoWayPlayer.decimal('SLG2019', 8, 3);
        twoWayPlayer.integer('PA2019');
        twoWayPlayer.integer('Doubles2019');
        twoWayPlayer.integer('HR2019');
        twoWayPlayer.integer('Runs2019');
        twoWayPlayer.integer('RBI2019');
        twoWayPlayer.string('LineupSlot2019', 50);
        twoWayPlayer.string('GBPercent2019', 50);
        twoWayPlayer.string('LDPercent2019', 50);
        twoWayPlayer.string('FBPercent2019', 50);
        twoWayPlayer.string('PullPercent2019', 50);
        twoWayPlayer.string('CenterPercent2019', 50);
        twoWayPlayer.string('OppoPercent2019', 50);
        twoWayPlayer.string('HRPerFB2019', 50);
        twoWayPlayer.string('HardHitPercent2019', 50);
        twoWayPlayer.decimal('ExitVelo2019', 8, 1);
        twoWayPlayer.decimal('FlyBallLineDriveExitVelo2019', 8, 1);
        twoWayPlayer.decimal('LaunchAngle2019', 8, 1);
        twoWayPlayer.string('BarrelPercent2019', 50);
        twoWayPlayer.decimal('BABIP2019', 8, 3);
        twoWayPlayer.decimal('WOBA2019', 8, 3);
        twoWayPlayer.decimal('xWOBA2019', 8, 3);
        twoWayPlayer.decimal('xBA2019', 8, 3);
        twoWayPlayer.integer('AvgHRDistance2019');
        twoWayPlayer.integer('WRCPlus2019');
        twoWayPlayer.integer('OPSPlus2019');
        twoWayPlayer.string('BBPercent2019', 50);
        twoWayPlayer.string('KPercent2019', 50);
        twoWayPlayer.string('ChaseRate2019', 50);
        twoWayPlayer.string('ContactPercent2019', 50);
        twoWayPlayer.decimal('SprintSpeed2019', 8, 1);
        twoWayPlayer.integer('StolenBases2019');
        twoWayPlayer.string('StolenBasePercent2019', 50);
        twoWayPlayer.decimal('FWAR2019', 8, 1);
        twoWayPlayer.integer('GamesPlayed2020');
        twoWayPlayer.decimal('BA2020', 8, 3);
        twoWayPlayer.decimal('OBP2020', 8, 3);
        twoWayPlayer.decimal('SLG2020', 8, 3);
        twoWayPlayer.integer('PA2020');
        twoWayPlayer.integer('Doubles2020');
        twoWayPlayer.integer('HR2020');
        twoWayPlayer.integer('Runs2020');
        twoWayPlayer.integer('RBI2020');
        twoWayPlayer.string('LineupSlot2020', 50);
        twoWayPlayer.string('GBPercent2020', 50);
        twoWayPlayer.string('LDPercent2020', 50);
        twoWayPlayer.string('FBPercent2020', 50);
        twoWayPlayer.string('PullPercent2020', 50);
        twoWayPlayer.string('CenterPercent2020', 50);
        twoWayPlayer.string('OppoPercent2020', 50);
        twoWayPlayer.string('HRPerFB2020', 50);
        twoWayPlayer.string('HardHitPercent2020', 50);
        twoWayPlayer.decimal('ExitVelo2020', 8, 1);
        twoWayPlayer.decimal('FlyBallLineDriveExitVelo2020', 8, 1);
        twoWayPlayer.decimal('LaunchAngle2020', 8, 1);
        twoWayPlayer.string('BarrelPercent2020', 50);
        twoWayPlayer.decimal('BABIP2020', 8, 3);
        twoWayPlayer.decimal('WOBA2020', 8, 3);
        twoWayPlayer.decimal('xWOBA2020', 8, 3);
        twoWayPlayer.decimal('xBA2020', 8, 3);
        twoWayPlayer.integer('AvgHRDistance2020');
        twoWayPlayer.integer('WRCPlus2020');
        twoWayPlayer.integer('OPSPlus2020');
        twoWayPlayer.string('BBPercent2020', 50);
        twoWayPlayer.string('KPercent2020', 50);
        twoWayPlayer.string('ChaseRate2020', 50);
        twoWayPlayer.string('ContactPercent2020', 50);
        twoWayPlayer.decimal('SprintSpeed2020', 8, 1);
        twoWayPlayer.integer('StolenBases2020');
        twoWayPlayer.string('StolenBasePercent2020', 50);
        twoWayPlayer.decimal('FWAR2020', 8, 1);
        twoWayPlayer.integer('GamesPlayed2021');
        twoWayPlayer.decimal('BA2021', 8, 3);
        twoWayPlayer.decimal('OBP2021', 8, 3);
        twoWayPlayer.decimal('SLG2021', 8, 3);
        twoWayPlayer.integer('PA2021');
        twoWayPlayer.integer('Doubles2021');
        twoWayPlayer.integer('HR2021');
        twoWayPlayer.integer('Runs2021');
        twoWayPlayer.integer('RBI2021');
        twoWayPlayer.string('LineupSlot2021', 50);
        twoWayPlayer.string('GBPercent2021', 50);
        twoWayPlayer.string('LDPercent2021', 50);
        twoWayPlayer.string('FBPercent2021', 50);
        twoWayPlayer.string('PullPercent2021', 50);
        twoWayPlayer.string('CenterPercent2021', 50);
        twoWayPlayer.string('OppoPercent2021', 50);
        twoWayPlayer.string('HRPerFB2021', 50);
        twoWayPlayer.string('HardHitPercent2021', 50);
        twoWayPlayer.decimal('ExitVelo2021', 8, 1);
        twoWayPlayer.decimal('FlyBallLineDriveExitVelo2021', 8, 1);
        twoWayPlayer.decimal('LaunchAngle2021', 8, 1);
        twoWayPlayer.string('BarrelPercent2021', 50);
        twoWayPlayer.decimal('BABIP2021', 8, 3);
        twoWayPlayer.decimal('WOBA2021', 8, 3);
        twoWayPlayer.decimal('xWOBA2021', 8, 3);
        twoWayPlayer.decimal('xBA2021', 8, 3);
        twoWayPlayer.integer('AvgHRDistance2021');
        twoWayPlayer.integer('WRCPlus2021');
        twoWayPlayer.integer('OPSPlus2021');
        twoWayPlayer.string('BBPercent2021', 50);
        twoWayPlayer.string('KPercent2021', 50);
        twoWayPlayer.string('ChaseRate2021', 50);
        twoWayPlayer.string('ContactPercent2021', 50);
        twoWayPlayer.decimal('SprintSpeed2021', 8, 1);
        twoWayPlayer.integer('StolenBases2021');
        twoWayPlayer.string('StolenBasePercent2021', 50);
        twoWayPlayer.decimal('FWAR2021', 8, 1);
        twoWayPlayer.integer('GamesPlayed2022');
        twoWayPlayer.decimal('BA2022', 8, 3);
        twoWayPlayer.decimal('OBP2022', 8, 3);
        twoWayPlayer.decimal('SLG2022', 8, 3);
        twoWayPlayer.integer('PA2022');
        twoWayPlayer.integer('Doubles2022');
        twoWayPlayer.integer('HR2022');
        twoWayPlayer.integer('Runs2022');
        twoWayPlayer.integer('RBI2022');
        twoWayPlayer.string('LineupSlot2022', 50);
        twoWayPlayer.string('GBPercent2022', 50);
        twoWayPlayer.string('LDPercent2022', 50);
        twoWayPlayer.string('FBPercent2022', 50);
        twoWayPlayer.string('PullPercent2022', 50);
        twoWayPlayer.string('CenterPercent2022', 50);
        twoWayPlayer.string('OppoPercent2022', 50);
        twoWayPlayer.string('HRPerFB2022', 50);
        twoWayPlayer.string('HardHitPercent2022', 50);
        twoWayPlayer.decimal('ExitVelo2022', 8, 1);
        twoWayPlayer.decimal('FlyBallLineDriveExitVelo2022', 8, 1);
        twoWayPlayer.decimal('LaunchAngle2022', 8, 1);
        twoWayPlayer.string('BarrelPercent2022', 50);
        twoWayPlayer.decimal('BABIP2022', 8, 3);
        twoWayPlayer.decimal('WOBA2022', 8, 3);
        twoWayPlayer.decimal('xWOBA2022', 8, 3);
        twoWayPlayer.decimal('xBA2022', 8, 3);
        twoWayPlayer.integer('AvgHRDistance2022');
        twoWayPlayer.integer('WRCPlus2022');
        twoWayPlayer.integer('OPSPlus2022');
        twoWayPlayer.string('BBPercent2022', 50);
        twoWayPlayer.string('KPercent2022', 50);
        twoWayPlayer.string('ChaseRate2022', 50);
        twoWayPlayer.string('ContactPercent2022', 50);
        twoWayPlayer.decimal('SprintSpeed2022', 8, 1);
        twoWayPlayer.integer('StolenBases2022');
        twoWayPlayer.string('StolenBasePercent2022', 50);
        twoWayPlayer.decimal('FWAR2022', 8, 1);
        twoWayPlayer.integer('TheBatXPAProjection');
        twoWayPlayer.decimal('TheBatXAVGProjection', 8, 3);
        twoWayPlayer.decimal('TheBatXOBPProjection', 8, 3);
        twoWayPlayer.decimal('TheBatXSLGProjection', 8, 3);
        twoWayPlayer.integer('TheBatXDoublesProjection', 50);
        twoWayPlayer.integer('TheBatXHRProjection', 50);
        twoWayPlayer.integer('TheBatXRunsProjection', 50);
        twoWayPlayer.integer('TheBatXRBIProjection', 50);
        twoWayPlayer.integer('TheBatXSBProjection');
        twoWayPlayer.decimal('SteamerFWARProjection', 8, 1);
        twoWayPlayer.integer('Games2017');
        twoWayPlayer.integer('GamesStarted2017');
        twoWayPlayer.decimal('InningsPitched2017', 8, 1);
        twoWayPlayer.decimal('InningsPitchedPerGameStarted2017', 8, 1);
        twoWayPlayer.integer('QS2017', 50);
        twoWayPlayer.decimal('ERA2017', 8, 2);
        twoWayPlayer.integer('ERAPlus2017');
        twoWayPlayer.decimal('FIP2017', 8, 2);
        twoWayPlayer.decimal('xFIP2017', 8, 2);
        twoWayPlayer.decimal('SIERA2017', 8, 2);
        twoWayPlayer.decimal('WHIP2017', 8, 2);
        twoWayPlayer.integer('RawKs2017');
        twoWayPlayer.decimal('HitsPer92017', 8, 2);
        twoWayPlayer.decimal('HRsPer92017', 8, 2);
        twoWayPlayer.decimal('BBPer92017', 8, 2);
        twoWayPlayer.decimal('KPer92017', 8, 2);
        twoWayPlayer.decimal('KDividedByBB2017', 8, 2);
        twoWayPlayer.integer('Saves2017');
        twoWayPlayer.string('PitcherKPercent2017', 50);
        twoWayPlayer.string('PitcherBBPercent2017', 50);
        twoWayPlayer.string('KMinusBBPercent2017', 50);
        twoWayPlayer.decimal('PitcherBABIP2017', 8, 3);
        twoWayPlayer.string('LOBPercent2017', 50);
        twoWayPlayer.string('HRPerFBPercent2017', 50);
        twoWayPlayer.string('PitcherGBPercent2017', 50);
        twoWayPlayer.string('PitcherLDPercent2017', 50);
        twoWayPlayer.string('PitcherFBPercent2017', 50);
        twoWayPlayer.string('IFFBPercent2017', 50);
        twoWayPlayer.string('SoftContactPercent2017', 50);
        twoWayPlayer.string('MediumContactPercent2017', 50);
        twoWayPlayer.string('HardContactPercent2017', 50);
        twoWayPlayer.string('FirstStrikePercent2017', 50);
        twoWayPlayer.decimal('PitcherExitVelo2017', 8, 1);
        twoWayPlayer.decimal('PitcherFlyBallLineDriveExitVelo2017', 8, 1);
        twoWayPlayer.string('PitcherBarrelPercent2017', 50);
        twoWayPlayer.decimal('PitcherLaunchAngle2017', 8, 1);
        twoWayPlayer.decimal('AVGFastballVelo2017', 8, 1);
        twoWayPlayer.decimal('MaxFastballVelo2017', 8, 1);
        twoWayPlayer.string('FourSeamFastballUsagePercent2017', 50);
        twoWayPlayer.string('SinkerUsagePercent2017', 50);
        twoWayPlayer.string('CutterUsagePercent2017', 50);
        twoWayPlayer.string('SliderUsagePercent2017', 50);
        twoWayPlayer.string('ChangeupUsagePercent2017', 50);
        twoWayPlayer.string('CurveUsagePercent2017', 50);
        twoWayPlayer.string('SplitterUsagePercent2017', 50);
        twoWayPlayer.string('KnuckleCurveUsagePercent2017', 50);
        twoWayPlayer.decimal('OverallXWOBA2017', 8, 3);
        twoWayPlayer.string('FourSeamFastballXWOBA2017', 50);
        twoWayPlayer.string('SinkerXWOBA2017', 50);
        twoWayPlayer.string('CutterXWOBA2017', 50);
        twoWayPlayer.string('SliderXWOBA2017', 50);
        twoWayPlayer.string('ChangeupXWOBA2017', 50);
        twoWayPlayer.string('CurveXWOBA2017', 50);
        twoWayPlayer.string('SplitterXWOBA2017', 50);
        twoWayPlayer.string('KnuckleCurveXWOBA2017', 50);
        twoWayPlayer.decimal('PitcherFWAR2017', 8, 1);
        twoWayPlayer.integer('Games2018');
        twoWayPlayer.integer('GamesStarted2018');
        twoWayPlayer.decimal('InningsPitched2018', 8, 1);
        twoWayPlayer.decimal('InningsPitchedPerGameStarted2018', 8, 1);
        twoWayPlayer.integer('QS2018', 50);
        twoWayPlayer.decimal('ERA2018', 8, 2);
        twoWayPlayer.integer('ERAPlus2018');
        twoWayPlayer.decimal('FIP2018', 8, 2);
        twoWayPlayer.decimal('xFIP2018', 8, 2);
        twoWayPlayer.decimal('SIERA2018', 8, 2);
        twoWayPlayer.decimal('WHIP2018', 8, 2);
        twoWayPlayer.integer('RawKs2018');
        twoWayPlayer.decimal('HitsPer92018', 8, 2);
        twoWayPlayer.decimal('HRsPer92018', 8, 2);
        twoWayPlayer.decimal('BBPer92018', 8, 2);
        twoWayPlayer.decimal('KPer92018', 8, 2);
        twoWayPlayer.decimal('KDividedByBB2018', 8, 2);
        twoWayPlayer.integer('Saves2018');
        twoWayPlayer.string('PitcherKPercent2018', 50);
        twoWayPlayer.string('PitcherBBPercent2018', 50);
        twoWayPlayer.string('KMinusBBPercent2018', 50);
        twoWayPlayer.decimal('PitcherBABIP2018', 8, 3);
        twoWayPlayer.string('LOBPercent2018', 50);
        twoWayPlayer.string('HRPerFBPercent2018', 50);
        twoWayPlayer.string('PitcherGBPercent2018', 50);
        twoWayPlayer.string('PitcherLDPercent2018', 50);
        twoWayPlayer.string('PitcherFBPercent2018', 50);
        twoWayPlayer.string('IFFBPercent2018', 50);
        twoWayPlayer.string('SoftContactPercent2018', 50);
        twoWayPlayer.string('MediumContactPercent2018', 50);
        twoWayPlayer.string('HardContactPercent2018', 50);
        twoWayPlayer.string('FirstStrikePercent2018', 50);
        twoWayPlayer.decimal('PitcherExitVelo2018', 8, 1);
        twoWayPlayer.decimal('PitcherFlyBallLineDriveExitVelo2018', 8, 1);
        twoWayPlayer.string('PitcherBarrelPercent2018', 50);
        twoWayPlayer.decimal('PitcherLaunchAngle2018', 8, 1);
        twoWayPlayer.decimal('AVGFastballVelo2018', 8, 1);
        twoWayPlayer.decimal('MaxFastballVelo2018', 8, 1);
        twoWayPlayer.string('FourSeamFastballUsagePercent2018', 50);
        twoWayPlayer.string('SinkerUsagePercent2018', 50);
        twoWayPlayer.string('CutterUsagePercent2018', 50);
        twoWayPlayer.string('SliderUsagePercent2018', 50);
        twoWayPlayer.string('ChangeupUsagePercent2018', 50);
        twoWayPlayer.string('CurveUsagePercent2018', 50);
        twoWayPlayer.string('SplitterUsagePercent2018', 50);
        twoWayPlayer.string('KnuckleCurveUsagePercent2018', 50);
        twoWayPlayer.decimal('OverallXWOBA2018', 8, 3);
        twoWayPlayer.string('FourSeamFastballXWOBA2018', 50);
        twoWayPlayer.string('SinkerXWOBA2018', 50);
        twoWayPlayer.string('CutterXWOBA2018', 50);
        twoWayPlayer.string('SliderXWOBA2018', 50);
        twoWayPlayer.string('ChangeupXWOBA2018', 50);
        twoWayPlayer.string('CurveXWOBA2018', 50);
        twoWayPlayer.string('SplitterXWOBA2018', 50);
        twoWayPlayer.string('KnuckleCurveXWOBA2018', 50);
        twoWayPlayer.decimal('PitcherFWAR2018', 8, 1);
        twoWayPlayer.integer('Games2019');
        twoWayPlayer.integer('GamesStarted2019');
        twoWayPlayer.decimal('InningsPitched2019', 8, 1);
        twoWayPlayer.decimal('InningsPitchedPerGameStarted2019', 8, 1);
        twoWayPlayer.integer('QS2019', 50);
        twoWayPlayer.decimal('ERA2019', 8, 2);
        twoWayPlayer.integer('ERAPlus2019');
        twoWayPlayer.decimal('FIP2019', 8, 2);
        twoWayPlayer.decimal('xFIP2019', 8, 2);
        twoWayPlayer.decimal('SIERA2019', 8, 2);
        twoWayPlayer.decimal('WHIP2019', 8, 2);
        twoWayPlayer.integer('RawKs2019');
        twoWayPlayer.decimal('HitsPer92019', 8, 2);
        twoWayPlayer.decimal('HRsPer92019', 8, 2);
        twoWayPlayer.decimal('BBPer92019', 8, 2);
        twoWayPlayer.decimal('KPer92019', 8, 2);
        twoWayPlayer.decimal('KDividedByBB2019', 8, 2);
        twoWayPlayer.integer('Saves2019');
        twoWayPlayer.string('PitcherKPercent2019', 50);
        twoWayPlayer.string('PitcherBBPercent2019', 50);
        twoWayPlayer.string('KMinusBBPercent2019', 50);
        twoWayPlayer.decimal('PitcherBABIP2019', 8, 3);
        twoWayPlayer.string('LOBPercent2019', 50);
        twoWayPlayer.string('HRPerFBPercent2019', 50);
        twoWayPlayer.string('PitcherGBPercent2019', 50);
        twoWayPlayer.string('PitcherLDPercent2019', 50);
        twoWayPlayer.string('PitcherFBPercent2019', 50);
        twoWayPlayer.string('IFFBPercent2019', 50);
        twoWayPlayer.string('SoftContactPercent2019', 50);
        twoWayPlayer.string('MediumContactPercent2019', 50);
        twoWayPlayer.string('HardContactPercent2019', 50);
        twoWayPlayer.string('FirstStrikePercent2019', 50);
        twoWayPlayer.decimal('PitcherExitVelo2019', 8, 1);
        twoWayPlayer.decimal('PitcherFlyBallLineDriveExitVelo2019', 8, 1);
        twoWayPlayer.string('PitcherBarrelPercent2019', 50);
        twoWayPlayer.decimal('PitcherLaunchAngle2019', 8, 1);
        twoWayPlayer.decimal('AVGFastballVelo2019', 8, 1);
        twoWayPlayer.decimal('MaxFastballVelo2019', 8, 1);
        twoWayPlayer.string('FourSeamFastballUsagePercent2019', 50);
        twoWayPlayer.string('SinkerUsagePercent2019', 50);
        twoWayPlayer.string('CutterUsagePercent2019', 50);
        twoWayPlayer.string('SliderUsagePercent2019', 50);
        twoWayPlayer.string('ChangeupUsagePercent2019', 50);
        twoWayPlayer.string('CurveUsagePercent2019', 50);
        twoWayPlayer.string('SplitterUsagePercent2019', 50);
        twoWayPlayer.string('KnuckleCurveUsagePercent2019', 50);
        twoWayPlayer.decimal('OverallXWOBA2019', 8, 3);
        twoWayPlayer.string('FourSeamFastballXWOBA2019', 50);
        twoWayPlayer.string('SinkerXWOBA2019', 50);
        twoWayPlayer.string('CutterXWOBA2019', 50);
        twoWayPlayer.string('SliderXWOBA2019', 50);
        twoWayPlayer.string('ChangeupXWOBA2019', 50);
        twoWayPlayer.string('CurveXWOBA2019', 50);
        twoWayPlayer.string('SplitterXWOBA2019', 50);
        twoWayPlayer.string('KnuckleCurveXWOBA2019', 50);
        twoWayPlayer.decimal('PitcherFWAR2019', 8, 1);
        twoWayPlayer.integer('Games2020');
        twoWayPlayer.integer('GamesStarted2020');
        twoWayPlayer.decimal('InningsPitched2020', 8, 1);
        twoWayPlayer.decimal('InningsPitchedPerGameStarted2020', 8, 1);
        twoWayPlayer.integer('QS2020', 50);
        twoWayPlayer.decimal('ERA2020', 8, 2);
        twoWayPlayer.integer('ERAPlus2020');
        twoWayPlayer.decimal('FIP2020', 8, 2);
        twoWayPlayer.decimal('xFIP2020', 8, 2);
        twoWayPlayer.decimal('SIERA2020', 8, 2);
        twoWayPlayer.decimal('WHIP2020', 8, 2);
        twoWayPlayer.integer('RawKs2020');
        twoWayPlayer.decimal('HitsPer92020', 8, 2);
        twoWayPlayer.decimal('HRsPer92020', 8, 2);
        twoWayPlayer.decimal('BBPer92020', 8, 2);
        twoWayPlayer.decimal('KPer92020', 8, 2);
        twoWayPlayer.decimal('KDividedByBB2020', 8, 2);
        twoWayPlayer.integer('Saves2020');
        twoWayPlayer.string('PitcherKPercent2020', 50);
        twoWayPlayer.string('PitcherBBPercent2020', 50);
        twoWayPlayer.string('KMinusBBPercent2020', 50);
        twoWayPlayer.decimal('PitcherBABIP2020', 8, 3);
        twoWayPlayer.string('LOBPercent2020', 50);
        twoWayPlayer.string('HRPerFBPercent2020', 50);
        twoWayPlayer.string('PitcherGBPercent2020', 50);
        twoWayPlayer.string('PitcherLDPercent2020', 50);
        twoWayPlayer.string('PitcherFBPercent2020', 50);
        twoWayPlayer.string('IFFBPercent2020', 50);
        twoWayPlayer.string('SoftContactPercent2020', 50);
        twoWayPlayer.string('MediumContactPercent2020', 50);
        twoWayPlayer.string('HardContactPercent2020', 50);
        twoWayPlayer.string('FirstStrikePercent2020', 50);
        twoWayPlayer.decimal('PitcherExitVelo2020', 8, 1);
        twoWayPlayer.decimal('PitcherFlyBallLineDriveExitVelo2020', 8, 1);
        twoWayPlayer.string('PitcherBarrelPercent2020', 50);
        twoWayPlayer.decimal('PitcherLaunchAngle2020', 8, 1);
        twoWayPlayer.decimal('AVGFastballVelo2020', 8, 1);
        twoWayPlayer.decimal('MaxFastballVelo2020', 8, 1);
        twoWayPlayer.string('FourSeamFastballUsagePercent2020', 50);
        twoWayPlayer.string('SinkerUsagePercent2020', 50);
        twoWayPlayer.string('CutterUsagePercent2020', 50);
        twoWayPlayer.string('SliderUsagePercent2020', 50);
        twoWayPlayer.string('ChangeupUsagePercent2020', 50);
        twoWayPlayer.string('CurveUsagePercent2020', 50);
        twoWayPlayer.string('SplitterUsagePercent2020', 50);
        twoWayPlayer.string('KnuckleCurveUsagePercent2020', 50);
        twoWayPlayer.decimal('OverallXWOBA2020', 8, 3);
        twoWayPlayer.string('FourSeamFastballXWOBA2020', 50);
        twoWayPlayer.string('SinkerXWOBA2020', 50);
        twoWayPlayer.string('CutterXWOBA2020', 50);
        twoWayPlayer.string('SliderXWOBA2020', 50);
        twoWayPlayer.string('ChangeupXWOBA2020', 50);
        twoWayPlayer.string('CurveXWOBA2020', 50);
        twoWayPlayer.string('SplitterXWOBA2020', 50);
        twoWayPlayer.string('KnuckleCurveXWOBA2020', 50);
        twoWayPlayer.decimal('PitcherFWAR2020', 8, 1);
        twoWayPlayer.integer('Games2021');
        twoWayPlayer.integer('GamesStarted2021');
        twoWayPlayer.decimal('InningsPitched2021', 8, 1);
        twoWayPlayer.decimal('InningsPitchedPerGameStarted2021', 8, 1);
        twoWayPlayer.integer('QS2021', 50);
        twoWayPlayer.decimal('ERA2021', 8, 2);
        twoWayPlayer.integer('ERAPlus2021');
        twoWayPlayer.decimal('FIP2021', 8, 2);
        twoWayPlayer.decimal('xFIP2021', 8, 2);
        twoWayPlayer.decimal('SIERA2021', 8, 2);
        twoWayPlayer.decimal('WHIP2021', 8, 2);
        twoWayPlayer.integer('RawKs2021');
        twoWayPlayer.decimal('HitsPer92021', 8, 2);
        twoWayPlayer.decimal('HRsPer92021', 8, 2);
        twoWayPlayer.decimal('BBPer92021', 8, 2);
        twoWayPlayer.decimal('KPer92021', 8, 2);
        twoWayPlayer.decimal('KDividedByBB2021', 8, 2);
        twoWayPlayer.integer('Saves2021');
        twoWayPlayer.string('PitcherKPercent2021', 50);
        twoWayPlayer.string('PitcherBBPercent2021', 50);
        twoWayPlayer.string('KMinusBBPercent2021', 50);
        twoWayPlayer.decimal('PitcherBABIP2021', 8, 3);
        twoWayPlayer.string('LOBPercent2021', 50);
        twoWayPlayer.string('HRPerFBPercent2021', 50);
        twoWayPlayer.string('PitcherGBPercent2021', 50);
        twoWayPlayer.string('PitcherLDPercent2021', 50);
        twoWayPlayer.string('PitcherFBPercent2021', 50);
        twoWayPlayer.string('IFFBPercent2021', 50);
        twoWayPlayer.string('SoftContactPercent2021', 50);
        twoWayPlayer.string('MediumContactPercent2021', 50);
        twoWayPlayer.string('HardContactPercent2021', 50);
        twoWayPlayer.string('FirstStrikePercent2021', 50);
        twoWayPlayer.decimal('PitcherExitVelo2021', 8, 1);
        twoWayPlayer.decimal('PitcherFlyBallLineDriveExitVelo2021', 8, 1);
        twoWayPlayer.string('PitcherBarrelPercent2021', 50);
        twoWayPlayer.decimal('PitcherLaunchAngle2021', 8, 1);
        twoWayPlayer.decimal('AVGFastballVelo2021', 8, 1);
        twoWayPlayer.decimal('MaxFastballVelo2021', 8, 1);
        twoWayPlayer.string('FourSeamFastballUsagePercent2021', 50);
        twoWayPlayer.string('SinkerUsagePercent2021', 50);
        twoWayPlayer.string('CutterUsagePercent2021', 50);
        twoWayPlayer.string('SliderUsagePercent2021', 50);
        twoWayPlayer.string('ChangeupUsagePercent2021', 50);
        twoWayPlayer.string('CurveUsagePercent2021', 50);
        twoWayPlayer.string('SplitterUsagePercent2021', 50);
        twoWayPlayer.string('KnuckleCurveUsagePercent2021', 50);
        twoWayPlayer.decimal('OverallXWOBA2021', 8, 3);
        twoWayPlayer.string('FourSeamFastballXWOBA2021', 50);
        twoWayPlayer.string('SinkerXWOBA2021', 50);
        twoWayPlayer.string('CutterXWOBA2021', 50);
        twoWayPlayer.string('SliderXWOBA2021', 50);
        twoWayPlayer.string('ChangeupXWOBA2021', 50);
        twoWayPlayer.string('CurveXWOBA2021', 50);
        twoWayPlayer.string('SplitterXWOBA2021', 50);
        twoWayPlayer.string('KnuckleCurveXWOBA2021', 50);
        twoWayPlayer.decimal('PitcherFWAR2021', 8, 1);
        twoWayPlayer.integer('Games2022');
        twoWayPlayer.integer('GamesStarted2022');
        twoWayPlayer.decimal('InningsPitched2022', 8, 1);
        twoWayPlayer.decimal('InningsPitchedPerGameStarted2022', 8, 1);
        twoWayPlayer.integer('QS2022', 50);
        twoWayPlayer.decimal('ERA2022', 8, 2);
        twoWayPlayer.integer('ERAPlus2022');
        twoWayPlayer.decimal('FIP2022', 8, 2);
        twoWayPlayer.decimal('xFIP2022', 8, 2);
        twoWayPlayer.decimal('SIERA2022', 8, 2);
        twoWayPlayer.decimal('WHIP2022', 8, 2);
        twoWayPlayer.integer('RawKs2022');
        twoWayPlayer.decimal('HitsPer92022', 8, 2);
        twoWayPlayer.decimal('HRsPer92022', 8, 2);
        twoWayPlayer.decimal('BBPer92022', 8, 2);
        twoWayPlayer.decimal('KPer92022', 8, 2);
        twoWayPlayer.decimal('KDividedByBB2022', 8, 2);
        twoWayPlayer.integer('Saves2022');
        twoWayPlayer.string('PitcherKPercent2022', 50);
        twoWayPlayer.string('PitcherBBPercent2022', 50);
        twoWayPlayer.string('KMinusBBPercent2022', 50);
        twoWayPlayer.decimal('PitcherBABIP2022', 8, 3);
        twoWayPlayer.string('LOBPercent2022', 50);
        twoWayPlayer.string('HRPerFBPercent2022', 50);
        twoWayPlayer.string('PitcherGBPercent2022', 50);
        twoWayPlayer.string('PitcherLDPercent2022', 50);
        twoWayPlayer.string('PitcherFBPercent2022', 50);
        twoWayPlayer.string('IFFBPercent2022', 50);
        twoWayPlayer.string('SoftContactPercent2022', 50);
        twoWayPlayer.string('MediumContactPercent2022', 50);
        twoWayPlayer.string('HardContactPercent2022', 50);
        twoWayPlayer.string('FirstStrikePercent2022', 50);
        twoWayPlayer.decimal('PitcherExitVelo2022', 8, 1);
        twoWayPlayer.decimal('PitcherFlyBallLineDriveExitVelo2022', 8, 1);
        twoWayPlayer.string('PitcherBarrelPercent2022', 50);
        twoWayPlayer.decimal('PitcherLaunchAngle2022', 8, 1);
        twoWayPlayer.decimal('AVGFastballVelo2022', 8, 1);
        twoWayPlayer.decimal('MaxFastballVelo2022', 8, 1);
        twoWayPlayer.string('FourSeamFastballUsagePercent2022', 50);
        twoWayPlayer.string('SinkerUsagePercent2022', 50);
        twoWayPlayer.string('CutterUsagePercent2022', 50);
        twoWayPlayer.string('SliderUsagePercent2022', 50);
        twoWayPlayer.string('ChangeupUsagePercent2022', 50);
        twoWayPlayer.string('CurveUsagePercent2022', 50);
        twoWayPlayer.string('SplitterUsagePercent2022', 50);
        twoWayPlayer.string('KnuckleCurveUsagePercent2022', 50);
        twoWayPlayer.decimal('OverallXWOBA2022', 8, 3);
        twoWayPlayer.string('FourSeamFastballXWOBA2022', 50);
        twoWayPlayer.string('SinkerXWOBA2022', 50);
        twoWayPlayer.string('CutterXWOBA2022', 50);
        twoWayPlayer.string('SliderXWOBA2022', 50);
        twoWayPlayer.string('ChangeupXWOBA2022', 50);
        twoWayPlayer.string('CurveXWOBA2022', 50);
        twoWayPlayer.string('SplitterXWOBA2022', 50);
        twoWayPlayer.string('KnuckleCurveXWOBA2022', 50);
        twoWayPlayer.decimal('PitcherFWAR2022', 8, 1);
        twoWayPlayer.integer('TheBatGamesProjection');
        twoWayPlayer.integer('TheBatGamesStartedProjection');
        twoWayPlayer.decimal('TheBatInningsPitchedProjection', 8, 1);
        twoWayPlayer.decimal('TheBatQSProjection', 8, 1);
        twoWayPlayer.decimal('TheBatERAProjection', 8, 2);
        twoWayPlayer.decimal('TheBatFIPProjection', 8, 2);
        twoWayPlayer.decimal('TheBatWHIPProjection', 8, 2);
        twoWayPlayer.integer('TheBatRawKsProjection');
        twoWayPlayer.decimal('TheBatKPer9Projection', 8, 2);
        twoWayPlayer.integer('TheBatSavesProjection');
        twoWayPlayer.decimal('PitcherSteamerFWARProjection', 8, 1);
    })
};

exports.down = function(knex, Promise) {
    return knex.schema.dropTableIfExists('twoWayPlayer');
};